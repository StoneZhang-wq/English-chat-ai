...
导入内容：user_profile、diary、npc_learn_progress（不导入 session_temp，其为临时数据）。
...
def main():
    ...
    for account_dir in sorted(accounts_dir.iterdir()):
        ...
        diary_path = account_dir / "diary.json"
        ...
        diary_data = {"version": "1.0", "last_updated": None, "entries": []}
        if diary_path.exists():
            try:
                diary_data = json.loads(diary_path.read_text(encoding="utf-8"))
            except Exception as e:
                print(f"  [{user_id}] 读取 diary 失败: {e}")
        npc_data = {}
        if npc_path.exists():
            try:
        ...
        try:
            ...
            client.table("user_diary").upsert({
                "user_id": user_id,
                "data": diary_data,
